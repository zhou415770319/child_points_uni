					...
					<view class="ai-detailed-content" v-if="detailedExpanded">
						<view class="detailed-item" v-for="(item, key) in parseAiDetailed(aiEvaluation.ai_detailed)" :key="key">
							<view class="item-header">
								...
								<text class="item-status" :class="getCorrectnessClass(item.correctness)">
									{{ getCorrectnessText(item.correctness) }}
								</text>
							</view>
		...
		methods: {
			...
			parseAiDetailed(detailedStr) {
				...
				try {
					...
					console.error('[Task Detail] 原始字符串长度:', detailedStr.length)
					console.error('[Task Detail] 原始字符串内容:', detailedStr)
					return null
				}
			},
